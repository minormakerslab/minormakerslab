---
layout: page
author: Daniël Put, Natalia Maślanka, Storm Nijhuis
title:  "Intro to Sewing Machines"
image: 
  path: /assets/images/workshops/sewing.jpg
  thumbnail: /assets/images/workshops/sewing.jpg
  caption: "sewing"
---


# Intro to Sewing Machines 
A  a fun and simple beginner’s guide to understanding and using sewing machines.

---

### What are you going to learn?

To introduce the sewing machine in a fun and simple way, helping beginners understand how it works, how to use it safely, and how to get started with basic sewing techniques. 

---

### What?

**Didactics**  
- **Competence**: Sewing machine operation
- **Theme**: Sewing
- **Learning objective**: 
    - Learn sewing machine parts and how they work
    - Learn how to thread and set up the machine
    - Practice sewing on paper
    - Try sewing two fabrics together

---

### Who, where, how long

- **Number of students**: between 5 and 30 students  
- **Location**: anywhere with electricity
- **Time**: 75 minutes  
  *(10 – 15 minutes per step)*

---

### Prepare and bring

- what to bring
    - sewing machines, thread, paperprintouts, fabric, pins, and tools 

---

### Want to know more about this topic?

**Book**  
- The Sewing Book by Alison Smith - a complete guide with photos 

**Articles**  
- “Sewing Machine Basics” - Threads Magazine 
- “Beginner’s Guide to Sewing Machines” - Tilly and the Buttons Blog 

**Other**  
- YouTube: Made to Sew channel 
- TikTok/Instagram: @coolstitches, @beginnersewing 
- Burda magazine

---

### How?

#### Step 1: Introduction & Safety (10 min)
- Welcome, overview, and basic safety tips
![welcom](/assets/images/workshops/Welcome.jpg)
![machine type](/assets/images/workshops/MachineType.jpg)
![Safety](/assets/images/workshops/Safety.jpg)
- Show parts of the sewing machine (IMG + machine)
![Machineparts](/assets/images/workshops/MachineParts.jpg)


---

#### Step 2: How It Works (15 min)
- Explain the sewing mechanism with video/GIF
![function](/assets/images/workshops/MachineFunction.jpg)
- Demonstrate threading the top thread and bobbin
![thread](/assets/images/workshops/practiceThread.jpg)



---

#### Step 3: First stitches on paper (20 min)
- Let participants work with the machine and explore
![practice](/assets/images/workshops/PracticeTime.jpg)
- Use given stencil to teach basic movements
![stencil](/assets/images/workshops/Stencil.jpg)
- *Warning*: set stitch length to atleast 3 when using paper





---

#### Step 4: Sewing fabric together (20 min)
- Use fabric | Teach backstitch
![fabric](/assets/images/workshops/2Fabric.jpg)
- See Troubleshooting when problems arise
![troubleshooting](/assets/images/workshops/Trouble.jpg)



---

#### Step 5: The End (5 min)
- Clean up and close the workshop
![thankyou](/assets/images/workshops/ThankYou.jpg)



---